    ...
    for product_set in products:
        ...
        for product in products_set:
            ...
            if exists:
                ...
                if float(existing_price) != float(product['Price']) or float(existing_quantity) != int(
                        product['FreeQuantityCenter']):
                    print('price updated')
                    update_product_price(product_id, product['Price'], product['PriceDiscount'],
                                         existing_product_data['quantity'])
            else:
                data = {
                    'id': product_id,
                    ...
                    'image_set': [{"name": 'https://happygifts.ru' + url} for url in
                                  product['Image']] if 'Image' in product else [],
                    'price': product['Price'],
